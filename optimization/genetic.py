...
class GeneticAlgorithm(OptimizationMethod):
    def __init__(self, p_cross1=0.6, p_cross2=0.3, p_mutation=0.1, elitism=0.1, population_size=20, region=None):
    ...
    def cross1(self, agent1, agent2):
        fit1 = self.handler.get_fintess(agent1)
        fit2 = self.handler.get_fintess(agent2)
    ...
    def call_methods(self):
        ...
        for i in range(2 * self.population_size):
            parents = self.selector.population(self.agents, 2, lambda x: 1.1 - self.handler.get_fintess(x))
        ...
        new_population = self.selector.population(new_population, self.population_size,
                                                  lambda x: 1.1 - self.handler.get_fintess(x))
